...
It is strongly recommended that you use [`isort`][isort] and some combination of
[`flake8`][flake8], [`autopep8`][autopep8], and [`unify`][unify] for consistency:

    pip install flake8 isort autopep8 unify
...
*   [`autopep8`][autopep8] is used to apply many PEP-8 changes automatically. Since some
    changes may have multiple acceptable options human review is recommended.
*   [`unify`][unify] normalizes the quote style in a file, making the use of single and
    double-quotes consistent throughout the project.
...
line length requirement this value would affect both [`flake8`][flake8]'s reports and
[`autopep8`][autopep8]'s formatting:
